...
afterEach(() => {
    ...
    if (originalKey === undefined)
        ...
        process.env.MARTA_API_KEY = originalKey;
});
...
function respondWithFeed(entity) {
...
test('Bus tolerates absent vehicle data', () => {
    ...
    const fields = ['trip', 'position', 'tripId', 'startDate', 'routeId', 'directionId',
        'latitude', 'longitude', 'bearing', 'timestamp', 'rawData'];
    for (const field of fields) {
        assert.equal(bus[field], undefined, field);
    }
});
for (const method of ['getTrains', 'getBuses']) {
